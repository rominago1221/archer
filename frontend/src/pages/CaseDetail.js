        ...
        <div style={{ background: '#fff', borderRight: '0.5px solid #e2e0db', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          ...
          <div style={{ margin: 10, padding: 11, background: '#eff6ff', borderRadius: 11, border: '0.5px solid #bfdbfe' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 4 }}>
              {[[t.stat1, t.stat1l], [t.stat2, t.stat2l], [t.stat3, t.stat3l], [t.stat4, t.stat4l]].map(([v, l], i) => (
                <div key={i} style={{ background: 'rgba(255,255,255,0.8)', borderRadius: 6, padding: '5px 7px', fontSize: 15, color: '#1e40af', fontWeight: 500, textAlign: 'center', lineHeight: 1.3 }}>
          ...
          <div style={{ padding: '4px 10px', display: 'flex', flexWrap: 'wrap', gap: 2 }}>
            {[{ icon: <Upload size={10} />, label: t.navUpload, to: '/upload' }, { icon: <Scale size={10} />, label: t.navLawyers, to: '/lawyers' }, { icon: <BookOpen size={10} />, label: t.navDocs, to: '/documents' }, { icon: <MessageSquare size={10} />, label: t.navChat, to: '/chat' }, { icon: <Settings size={10} />, label: t.navSettings, to: '/settings' }].map((n, i) => (
              <button key={i} onClick={() => navigate(n.to)} style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '3px 7px', fontSize: 15, color: '#6b7280', background: 'none', border: 'none', cursor: 'pointer', borderRadius: 5 }}>{n.icon}{n.label}</button>
            ))}
          </div>
          {/* Cases */}
              ...
              <>
                ...
                <div style={{ background: '#fff', borderRadius: 12, padding: '14px 18px', marginBottom: 10, border: '0.5px solid #e2e0db' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 10 }}>
                    <div style={{ width: 22, height: 22, borderRadius: '50%', background: '#1a56db', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700, color: '#fff' }}>J</div>
                    <div style={{ fontSize: 15, fontWeight: 600, color: '#1a1a2e' }}>{t.analysisTitle}</div>
                    <div style={{ marginLeft: 'auto', fontSize: 15, color: '#9ca3af', display: 'flex', alignItems: 'center', gap: 4 }}>
                  ...
                  {findings.map((f, i) => {
                    ...
                    return (
                      <div key={i} style={{ padding: '9px 0', borderBottom: i < findings.length - 1 ? '0.5px solid #f3f4f6' : 'none', display: 'flex', alignItems: 'flex-start', gap: 7 }}>
                        ...
                        <div style={{ flex: 1 }}>
                          <div style={{ fontSize: 15, color: '#374151', lineHeight: 1.55 }} dangerouslySetInnerHTML={{ __html: (f.text || '').replace(/\*\*(.*?)\*\*/g, '<b style="color:#1a1a2e;font-weight:600">$1</b>') }} />
                    ...
                    <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 8 }}>
                      {(jq.options || []).slice(0, 3).map((opt, i) => (
                        <button key={i} data-testid={`james-answer-${i}`} onClick={() => handleJamesAnswer(opt)} disabled={answerLoading}
                          style={{ padding: '6px 14px', background: answerLoading ? '#f3f4f6' : '#fff', color: answerLoading ? '#9ca3af' : '#1a1a2e', border: '0.5px solid #e2e0db', borderRadius: 8, fontSize: 12, fontWeight: 500, cursor: answerLoading ? 'default' : 'pointer' }}>
                          {opt}
                        </button>
                      ))}
                    </div>
                    <button data-testid="ask-james-directly" onClick={() => setChatDrawer({ initial: `I have a question about my case "${sc?.title}". ${jq.text}` })}
                      style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 12, color: '#1a56db', fontWeight: 500, padding: 0 }}>
                      {t.moreQuestions} →
                    </button>
                  </div>
                )}
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                      <div style={{ background: '#f0fdf4', border: '0.5px solid #86efac', borderRadius: 9, padding: 10 }}>
                        ...
                        {(bp.user_side?.strongest_arguments || bp.user_arguments || []).slice(0, 5).map((a, i) => (
                          <div key={i} style={{ fontSize: 12, color: '#374151', padding: '4px 0', borderBottom: '0.5px solid rgba(0,0,0,0.05)', lineHeight: 1.5, display: 'flex', gap: 6 }}>
